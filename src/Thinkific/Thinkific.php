...
class Thinkific {
	...
	public function request( $options ) {
		...
		try {
			...
			throw new \Exception("Error while trying to " . $method . ' ' . $url . " --> " . $ex->getCode() . " --> " . $ex->getMessage() . "\n" . $ex->getResponse()->getBody(), $ex->getCode());
		} catch ( GuzzleException $e ) {
			throw new \Exception("Error while trying to " . $method . ' ' . $url . " --> " . $e->getCode() . " --> " . $e->getMessage(), $ex->getCode());
		} catch ( \Exception $e ) {
			throw new \Exception("Error while trying to " . $method . ' ' . $url . " --> " . $e->getCode() . " --> " . $e->getMessage(), $ex->getCode());
		}
	}
